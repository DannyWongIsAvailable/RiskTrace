...
def get_run_events(
  ...
  view: Literal["raw", "trajectory"] = Query(default="raw"),
  authorization: str | None = Header(default=None),
) -> dict[str, Any]:
  """Read canonical events or a lightweight UI trajectory projection incrementally."""
  _verify_authorization(authorization)
  try:
    return _manager(request).get_events(
      run_id,
      after_seq=after,
      limit=limit,
      view=view,
