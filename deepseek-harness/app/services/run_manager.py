...
TRAJECTORY_EVENT_PAGE_LIMIT = 5000
...
class RunManager:
  ...
  def get_events(
    ...
    *,
    ...
    if view == "trajectory":
      max_limit = TRAJECTORY_EVENT_PAGE_LIMIT
      event_types = TRAJECTORY_EVENT_TYPES
    else:
      max_limit = (
        LIVE_EVENT_PAGE_LIMIT
        if snapshot.get("status") in {"queued", "running"}
        else COMPLETED_EVENT_PAGE_LIMIT
      )
      event_types = None

    effective_limit = max(1, min(max_limit, int(limit)))
    events, has_more, next_seq = self.store.list_events(
      run_id,
      after_seq=after_seq,
